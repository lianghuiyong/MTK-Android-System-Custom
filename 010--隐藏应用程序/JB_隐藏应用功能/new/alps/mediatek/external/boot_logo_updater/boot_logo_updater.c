...
int getHideFlag(unsigned char *result)
{
	...
	SLOGE("file: %s, %s_chenmin(%d)_<getHideFlag>, result = %c \n", __FILE__, __func__, __LINE__, *result);
...
//    fbsize = finfo.line_length * vinfo.yres;
	...
	if (!hideflag)
	{   
		SLOGE("file: %s, %s_chenmin(%d)_<mian>_default, hideflag = %d \n", __FILE__, __func__, __LINE__, hideflag);  
	...
	{     
		SLOGE("file: %s, %s_chenmin(%d)_<main>_customer, hideflag = %d \n", __FILE__, __func__, __LINE__, hideflag);  
		if (strcmp(hidebuff,"unknown") == 0){
